...
export default function DebugAdminPage() {
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-[var(--color-warm-white)] text-[var(--color-navy)] p-6">
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="rounded-2xl border border-[var(--color-border-light)] bg-[var(--color-warm-cream)]/85 p-6 space-y-4">
          ...
          <div className="text-xs space-y-1 font-mono">
            <div>Email: <span className="text-[var(--color-slate)]">{user.email}</span></div>
            <div>User ID: <span className="text-[var(--color-slate)]">{user.id}</span></div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[var(--color-border-light)] bg-[var(--color-warm-cream)]/85 p-6 space-y-4">
          ...
          {error ? (
            <div className="text-xs text-red-300 space-y-2">
              ...
              <div className="mt-4 p-3 bg-white rounded border border-[var(--color-border)]">
                <p className="text-[11px] font-semibold mb-2">Fix in Supabase SQL Editor:</p>
                <pre className="text-[10px] font-mono text-[var(--color-slate)] overflow-x-auto">
...
ALTER TABLE public.profiles DISABLE ROW LEVEL SECURITY;`}
            ...
            <div className="text-xs space-y-2">
              <div className="grid grid-cols-2 gap-2">
                <div>ID:</div>
                <div className="font-mono text-[var(--color-slate)]">{profileData.id}</div>
                <div>Role:</div>
                <div className="text-[var(--color-slate)]">{profileData.role || "—"}</div>
                <div>is_admin:</div>
                <div className={profileData.is_admin ? "text-emerald-400 font-semibold" : "text-red-400"}>
                  {String(profileData.is_admin)}
                </div>
                <div>Organization:</div>
                <div className="text-[var(--color-slate)]">{profileData.organization || "—"}</div>
              </div>
...
WHERE id = '${user.id}';`}
            ...
            <div className="text-xs text-[var(--color-muted)]">Loading profile...</div>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-[var(--color-border-light)] bg-[var(--color-warm-cream)]/85 p-6 space-y-4">
          ...
          <div className="text-xs space-y-2">
            ...
            <pre className="p-3 bg-white rounded border border-[var(--color-border)] text-[10px] font-mono text-[var(--color-slate)] overflow-x-auto">
