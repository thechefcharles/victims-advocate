    ...
    <main className={APP_PAGE_MAIN}>
      <div className="relative max-w-2xl mx-auto space-y-6">
        ...
        {!loading ? (
          <div className={`${APP_CARD} space-y-3`}>
            <div>
              ...
              <p className="mt-1 text-xs text-[var(--color-muted)]">{t("victimDashboard.caseOrgManage.referralUpdatesIntro")}</p>
            </div>
            {referrals.length === 0 ? (
              <p className="text-sm text-[var(--color-muted)]">{t("victimDashboard.caseOrgManage.referralUpdatesEmpty")}</p>
            ) : (
        ...
        <p className="text-[11px] text-[var(--color-muted)]">
          <Link href={victimCasePaths.advocate(caseId)} className="text-[var(--color-muted)] hover:text-[var(--color-charcoal)] underline">
