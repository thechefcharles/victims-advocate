/**
 * Org admin / supervisor approves an org rep's join request (creates membership as org_admin).
 */
  ...
  try {
    ...
    if (memErr) {
      if (memErr.code === "23505") {
        return apiFail("VALIDATION_ERROR", "This user already has a membership", undefined, 409);
      }
    ...
    await createNotification(
      {
        ...
        body: `You've been added to ${orgName} as an organization admin. Open your dashboard to continue.`,
    ...
    await logEvent({
      ctx,
      action: "org.rep_join_request.approved",
    ...
    logger.error("org.rep_join_request.approve.error", {
      code: appErr.code,
