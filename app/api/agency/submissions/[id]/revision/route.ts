/**
 * POST /api/agency/submissions/[id]/revision — request revision (agency officer/owner)
 */
...
import { requestReportingRevision } from "@/lib/server/agency/reportingSubmissionService";
...
export async function POST(req: Request, ctxParams: RouteParams) {
  try {
    ...
    const decision = await can("reporting_submission:request_revision", actor, {
    ...
    const updated = await requestReportingRevision({
    ...
    if (appErr.code === "INTERNAL") logger.error("agency.revision.error", { code: appErr.code, message: appErr.message });
