/**
 * POST /api/agency/submissions/[id]/accept — accept submission (agency officer/owner ONLY)
 */
...
export async function POST(req: Request, ctxParams: RouteParams) {
  try {
    ...
    const decision = await can("reporting_submission:accept", actor, {
    ...
    const updated = await acceptReportingSubmission({
      submissionId: id, reviewerUserId: ctx.userId, supabase,
    });
    return apiOk({ submission: serializeSubmissionForAgency(updated) });
  } catch (err) {
    const appErr = toAppError(err);
    if (appErr.code === "INTERNAL") logger.error("agency.accept.error", { code: appErr.code, message: appErr.message });
