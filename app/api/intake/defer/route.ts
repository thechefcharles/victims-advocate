...
import { getFieldStateMap, setFieldState, makeDeferredEntry, mergeFieldState, stripFieldState } from "@/lib/intake/fieldState";
import { canDefer } from "@/lib/intake/fieldConfig";
...
export async function POST(req: Request) {
  try {
    ...
    if (!canDefer(fieldKey)) {
      return apiFail("VALIDATION_ERROR", "This field does not allow defer", undefined, 422);
    }
    ...
    const newState = setFieldState(stateMap, fieldKey, makeDeferredEntry("user"));
    ...
    if (orgId) {
      appendCaseTimelineEvent({
        ...
        eventType: "case.intake_field_deferred",
        title: "Intake question deferred",
        description: null,
    ...
    logger.info("intake.defer", { caseId, fieldKey, userId: ctx.userId });
    return NextResponse.json({ ok: true, data: { application: updated } });
  } catch (err) {
    const appErr = toAppError(err);
    logger.error("intake.defer.error", { code: appErr.code, message: appErr.message });
