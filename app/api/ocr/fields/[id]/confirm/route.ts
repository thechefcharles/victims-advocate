/**
 * Phase 13: Confirm an extracted OCR field (mark as reviewed and accepted).
 */
...
export async function POST(req: Request, context: RouteParams) {
  try {
    ...
    const row = field as { case_id: string; organization_id: string };
    ...
    logger.info("ocr.field.confirm", { fieldId, userId: ctx.userId });
    return apiOk({ field: updated });
  } catch (err) {
    const appErr = toAppError(err);
    logger.error("ocr.fields.confirm.error", { code: appErr.code, message: appErr.message });
