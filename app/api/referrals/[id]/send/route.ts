...
import { getReferral, sendReferral } from "@/lib/server/referrals/referralService";
...
export async function POST(req: Request, context: RouteParams) {
  try {
    ...
    const updated = await sendReferral({ ctx, id: id.trim() });
    return apiOk({ referral: serializeForSourceOrg(updated) });
  } catch (err) {
    const appErr = toAppError(err);
    if (appErr.code === "INTERNAL") {
      logger.error("referrals.send.error", { code: appErr.code, message: appErr.message });
    }
