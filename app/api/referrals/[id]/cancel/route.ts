...
import { getReferral, cancelReferral } from "@/lib/server/referrals/referralService";
...
export async function POST(req: Request, context: RouteParams) {
  try {
    ...
    const decision = await can("referral:cancel", actor, {
      type: "referral",
      id: referral.id,
    ...
    const updated = await cancelReferral({ ctx, id: id.trim() });
    return apiOk({ referral: serializeForSourceOrg(updated) });
  } catch (err) {
    const appErr = toAppError(err);
    if (appErr.code === "INTERNAL") {
      logger.error("referrals.cancel.error", { code: appErr.code, message: appErr.message });
    }
