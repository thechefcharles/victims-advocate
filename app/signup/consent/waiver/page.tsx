...
import { LiabilityWaiverConsentDocument } from "@/components/legal/documents/LiabilityWaiverConsentDocument";
...
export default function SignupConsentWaiverPage() {
  ...
  const onContinue = async () => {
    ...
    try {
      ...
      if (getPlatformStatus() === "production") {
        ...
        router.push(`/signup/consent/beta?redirect=${encodeURIComponent(redirectTo)}`);
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <SignupConsentChrome steps={steps} activeStepId="waiver">
      ...
      <LegalDocumentScrollRegion onReachedEnd={setScrollOk}>
        <LiabilityWaiverConsentDocument />
      ...
      <div className="space-y-4" aria-live="polite">
        <label className="flex min-h-[44px] cursor-pointer items-start gap-3 text-base text-[var(--color-charcoal)]">
          <input
            ...
            aria-label="I have read and agree to the NxtStps Liability Waiver and Release of Claims"
          />
          <span>I have read and agree to the NxtStps Liability Waiver and Release of Claims</span>
        </label>
