 ...
 * Canonical 7-state lifecycle from Master System Document.
 */
...
// ---------------------------------------------------------------------------
// Domain 6.1 — SignalDispute
// ---------------------------------------------------------------------------

/**
 * Owned by: Domain 6.1 Trust / Transparency / Scoring — SignalDispute workflow.
 * Canonical values from the Dispute Resolution Workflow spec.
 * Signals are never edited/deleted; removal means excluded_from_scoring.
 */
export type SignalDisputeStatus =
  | "submitted"
  | "under_review"
  | "resolved_upheld"
  | "resolved_annotated"
 ...
 * Canonical 3-state lifecycle from Master System Document:
 *   active    — session in progress, normal guidance mode
 *   escalated — Category 1 safety/crisis trigger fired; guidance suspended
 *   closed    — session ended (user closed or soft-closed after timeout)
 */
...
// ---------------------------------------------------------------------------
// Domain 7.3 — AdvocateCopilotDraft
// ---------------------------------------------------------------------------

/**
 * Owned by: Domain 7.3 AI Guidance — provider copilot output.
 * Canonical 4-state lifecycle from Master System Document:
 *   draft_generated → reviewed → applied | discarded
 * No auto-apply in v1 — human review is mandatory before applied.
 */
