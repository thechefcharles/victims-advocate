 ...
 * - Backfills missing `applicant_connection_request_pending` rows (e.g. requests sent before we created them).
 * - Dismisses stray `advocate_connection_accepted` notifications whose request_id still has status pending (bad state).
 */
export async function syncApplicantConnectionNotifications(ctx: AuthContext): Promise<void> {
  ...
  for (const row of pendingRows) {
    ...
    const created = await createNotification(
      {
        userId: ctx.userId,
        type: "applicant_connection_request_pending",
        title,
        body,
