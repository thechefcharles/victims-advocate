/**
 * After an applicant removes an advocate from a case (case_access deleted), align
...
export async function syncConnectionRequestsAfterApplicantRemovesAdvocateFromCase(params: {
  victimUserId: string;
  ...
  if (stillConnected) {
    const { error } = await supabase
      ...
      .delete()
      .eq("applicant_user_id", victimUserId)
