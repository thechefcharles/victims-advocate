  ...
  return (
    <>
      {pendingRemove && (
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-[var(--color-border)] bg-white p-5 shadow-xl shadow-[var(--shadow-modal)]">
            <h3 id="remove-client-title" className="text-base font-semibold text-white">
              Remove this client?
            </h3>
            <p className="mt-2 text-sm text-[var(--color-slate)]">
              <span className="font-medium text-[var(--color-navy)]">{pendingRemove.display_name}</span> will
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-2">
              <button
                ...
                className="rounded-full border border-[var(--color-border)] px-4 py-2 text-sm font-medium text-[var(--color-charcoal)] hover:bg-[var(--color-light-sand)] disabled:opacity-50"
              >
                Cancel
              </button>
              <button
          ...
          <div className="grid gap-3">
            {clients.map((c) => (
              <div
                ...
                className="flex flex-col gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-light-sand)]/85 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-3"
              >
                <Link
                  href={`/dashboard/clients/${c.client_user_id}`}
                  className="min-w-0 flex-1 hover:bg-[var(--color-light-sand)]/70 -mx-1 px-1 py-0.5 rounded-lg transition"
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="text-[11px] text-[var(--color-slate)] shrink-0 sm:hidden">Open →</div>
                  </div>
                </Link>
                <div className="flex items-center gap-2 shrink-0 justify-end">
                  <Link
                    href={`/dashboard/clients/${c.client_user_id}`}
                    className="text-[11px] text-[var(--color-slate)] hover:text-white hidden sm:inline"
                  >
                    Open →
                  </Link>
                  <button
