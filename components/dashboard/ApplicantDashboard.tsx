// components/dashboard/ApplicantDashboard.tsx
"use client";

import { useCallback, useEffect, useMemo, useState } from "react";
import type { FunnelStepId } from "@/lib/applicantDashboardFunnel";
...
} from "@/lib/applicantDashboardFunnel";
import { ApplicantFunnelStepper } from "@/components/dashboard/ApplicantFunnelStepper";
import { ApplicantProfileCompletionBanner } from "@/components/dashboard/ApplicantProfileCompletionBanner";
...
export default function ApplicantDashboard({
  ...
  const dashboardHeaderTitle = useMemo(() => {
    ...
    if (name) return tf("applicantDashboard.welcomeTitle", { name });
    return t("applicantDashboard.title");
  }, [personalInfo, t, tf]);
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      ...
      console.error(e);
      setErr(t("applicantDashboard.loadError"));
    } finally {
      setLoading(false);
    }
  }, [token, router, t]);
  ...
  const funnelLabels = useMemo(
    () => ({
      eligibility: t("applicantDashboard.funnel.stepEligibility"),
      application: t("applicantDashboard.funnel.stepApplication"),
      support: t("applicantDashboard.funnel.stepSupport"),
    }),
    [t]
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/applicant/support-overview", {
          headers: { Authorization: `Bearer ${token}` },
        });
    ...
    async (programState: "IL" | "IN") => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          let msg = t("applicantDashboard.loadError");
          try {
        ...
        console.error(e);
        setErr(e instanceof Error && e.message ? e.message : t("applicantDashboard.loadError"));
      } finally {
        setCreatingCase(false);
      }
    },
  ...
  const confirmDeleteCase = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDeleteErr(t("applicantDashboard.deleteFailed"));
      }
    } catch {
      setDeleteErr(t("applicantDashboard.deleteFailed"));
    } finally {
  ...
  const findOrgHref = useMemo(
    () =>
      focusCaseId
        ? `${ROUTES.applicantFindOrganizations}?case=${encodeURIComponent(focusCaseId)}`
        : ROUTES.applicantFindOrganizations,
    [focusCaseId]
  );

  const manageOrgPath = focusCaseId ? applicantCasePaths.organization(focusCaseId) : findOrgHref;
  const manageAdvocatePath = focusCaseId ? applicantCasePaths.advocate(focusCaseId) : connectAdvocateHref;
  ...
  const messagesRowStatus = !focusCaseId
    ? t("applicantDashboard.messagesEmpty")
    : secondaryLoading
      ? t("applicantDashboard.messagesLoading")
      : messagesUnread > 0
        ? messagesUnread === 1
          ? t("applicantDashboard.messagesUnreadOne")
          : tf("applicantDashboard.messagesUnreadMany", { count: messagesUnread })
        : messagesTotal === 0
          ? t("applicantDashboard.messagesEmpty")
          : t("applicantDashboard.messagesInThread");

  const documentsRowStatus = !focusCaseId
    ? t("applicantDashboard.documentsNoCase")
    : secondaryLoading
      ? t("applicantDashboard.updatingDetails")
      : completenessResult && hasBlockingDocumentGap(completenessResult)
        ? t("applicantDashboard.documentsStatusMissing")
        : t("applicantDashboard.documentsStatusGeneric");

  const appointmentsRowStatus = !focusCaseId
    ? t("applicantDashboard.appointmentsEmpty")
    : secondaryLoading
      ? t("applicantDashboard.updatingDetails")
      : t("applicantDashboard.appointmentsEmpty");

  const supportRowStatus = !focusCaseId
    ? t("applicantDashboard.supportNoCase")
    : secondaryLoading
      ? t("applicantDashboard.updatingDetails")
      : matchCount === 0
        ? t("applicantDashboard.supportNoMatches")
        : matchCount === 1
          ? t("applicantDashboard.supportMatchOne")
          : tf("applicantDashboard.supportMatchMany", { count: matchCount });
  ...
  const renderHubMainCta = () => {
    ...
    return (
      <Link href={href} className={`${primaryCtaClass} ${priorityRingClassName("medium")}`}>
        {t("applicantDashboard.resumeApplication")}
      </Link>
    );
  };
      ...
      <div className="relative max-w-3xl mx-auto space-y-6">
        {loading && !err ? (
          ...
          <>
                  ...
                  cases.length > 0 && focusCase ? (
                    <div className="flex flex-wrap items-center gap-2 justify-end">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {t("applicantDashboard.myCasesSectionLabel")}
                        </span>
                        <select
                      ...
                      >
                        {creatingCase ? t("applicantDashboard.creating") : t("applicantDashboard.newCaseButton")}
                      </button>
                    </div>
                  ) : null
                }
            ...
            >
              <ApplicantFunnelStepper
                variant="minimal"
                steps={funnelSteps}
                labels={funnelLabels}
                ariaLabel={t("applicantDashboard.funnel.ariaLabel")}
                title={t("applicantDashboard.progressTitle")}
            ...
            <PrimaryActionArea
              ariaLabel={t("applicantDashboard.applyResumeCardAria")}
              className="shadow-sm"
              surface="neutral"
              eyebrow={false}
              primary={
                  ...
                  {(showNextStepConnectAdvocate || showNextStepFindOrganizations) && (
                    <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
                      {showNextStepConnectAdvocate ? (
                        ...
                        >
                          {t("applicantDashboard.getHelp.connectAdvocate")}
                        </Link>
                      ) : null}
                      {showNextStepFindOrganizations ? (
                        ...
                        >
                          {t("applicantDashboard.getHelp.findOrganizations")}
                        </Link>
                      ) : null}
                    </div>
                  )}
            ...
            >
              ...
              >
                {t("applicantDashboard.supportTeamTitle")}
              </h2>
              {noCasesYet ? (
                <p className="mt-1 text-[11px] text-[var(--color-muted)] leading-relaxed">
                  {t("applicantDashboard.supportTeamNoCaseHint")}
                </p>
              ) : null}
              {supportSectionLoading ? (
                <p className="mt-2 text-[11px] text-[var(--color-slate)]">{t("applicantDashboard.supportTeamLoading")}</p>
              ) : (
                <div className="mt-2 grid gap-3 sm:grid-cols-2">
                  <div>
                    <p className="text-xs font-medium text-[var(--color-slate)]">{t("applicantDashboard.supportTeamOrg")}</p>
                    <div className="mt-1.5 space-y-3">
                      {caseLinkedOrgName ? (
                        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
                          <Link
                            href={manageOrgPath}
                            title={t("applicantDashboard.supportTeamEditOrgTitle")}
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            >
                              {t("applicantDashboard.supportTeamContactOrg")}
                            </Link>
                          </div>
                        </div>
                      ) : null}
                      {hasPendingOrgConnects ? (
                        <div className="rounded-lg border border-amber-500/25 bg-amber-950/15 px-3 py-2">
                          <p className="text-[11px] font-medium text-amber-200/95">
                            {t("applicantDashboard.supportTeamPendingOrgConnectsTitle")}
                          </p>
                      ...
                      {!caseLinkedOrgName && !hasPendingOrgConnects ? (
                        <div className="space-y-2">
                          <p className="text-[11px] text-[var(--color-muted)] leading-relaxed">
                            {t("applicantDashboard.supportTeamNoOrg")}
                          </p>
                          <Link
                            href={findOrgHref}
                          ...
                          >
                            {t("applicantDashboard.supportTeamAddOrgCta")}
                          </Link>
                        </div>
                      ) : null}
                    </div>
                  ...
                  <div>
                    <p className="text-xs font-medium text-[var(--color-slate)]">{t("applicantDashboard.supportTeamAdvocates")}</p>
                    {focusCaseId && supportTeam?.advocates && supportTeam.advocates.length > 0 ? (
                      <div className="mt-0.5 space-y-2">
                            ...
                            <li key={a.id}>
                              <div className="flex flex-col gap-1.5 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                                <Link
                                  ...
                                  title={t("applicantDashboard.supportTeamEditAdvocateTitle")}
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                  <Link
                                    ...
                                    className="inline-flex items-center justify-center rounded-full border border-[var(--color-border)] bg-[var(--color-warm-cream)]/80 px-2.5 py-1 text-[10px] font-semibold text-[var(--color-charcoal)] hover:bg-[var(--color-light-sand)]/85"
                                  >
                                    {t("applicantDashboard.supportTeamSendMessage")}
                                  </Link>
                                </div>
                              </div>
                            </li>
                        ...
                        {advocatePendingEffective && hasPendingAdvocateDetail ? (
                          <div className="rounded-lg border border-amber-500/25 bg-amber-950/15 px-3 py-2">
                            <p className="text-[11px] font-medium text-amber-200/95">
                              {t("applicantDashboard.supportTeamAdvocateMorePending")}
                            </p>
                            <ul className="mt-1 list-none space-y-0.5 text-[11px] text-[var(--color-slate)]">
                      ...
                      <div className="mt-1.5 space-y-2">
                        <p className="text-[11px] font-medium text-amber-200/90">
                          {t("applicantDashboard.supportTeamAdvocateRequestPending")}
                        </p>
                      ...
                      <div className="mt-1.5 space-y-1.5">
                        <p className="text-[11px] text-[var(--color-slate)]">{t("applicantDashboard.supportTeamNoAdvocates")}</p>
                        <Link
                          href={connectAdvocateHref}
                        ...
                        >
                          {t("applicantDashboard.supportTeamConnectCta")}
                        </Link>
                      </div>
                    )}
                  </div>
            ...
            {!noCasesYet && focusCaseId && focusCase && (
              <section className={`${APP_CARD} border-[var(--color-border-light)] bg-[var(--color-warm-cream)]/65 space-y-1`}>
                <h2 className="text-sm font-semibold text-[var(--color-navy)] pb-2">
                  {t("applicantDashboard.caseDetailsHeading")}
                </h2>
                <div className="divide-y divide-[var(--color-border-light)] rounded-xl border border-[var(--color-border-light)] overflow-hidden">
                  ...
                  >
                    <span className="font-medium text-[var(--color-navy)]">{t("applicantDashboard.caseActivityMessages")}</span>
                  ...
                  <Link
                    ...
                    className="flex items-center justify-between gap-3 px-3 py-3 text-sm text-[var(--color-charcoal)] hover:bg-[var(--color-warm-cream)]/80 transition-colors"
                  >
                    <span className="font-medium text-[var(--color-navy)]">{t("applicantDashboard.caseActivityDocuments")}</span>
                  ...
                  <Link
                    ...
                    className="flex items-center justify-between gap-3 px-3 py-3 text-sm text-[var(--color-charcoal)] hover:bg-[var(--color-warm-cream)]/80 transition-colors"
                  >
                    <span className="font-medium text-[var(--color-navy)]">{t("applicantDashboard.caseActivityAppointments")}</span>
                  ...
                  <Link
                    ...
                    className="flex items-center justify-between gap-3 px-3 py-3 text-sm text-[var(--color-charcoal)] hover:bg-[var(--color-warm-cream)]/80 transition-colors"
                  >
                    <span className="font-medium text-[var(--color-navy)]">{t("applicantDashboard.caseActivitySupport")}</span>
              ...
              {applyModalStep === "path" ? (
                <>
                  <h2 id="apply-modal-title" className="sr-only">
                    {t("applicantDashboard.applyPathAria")}
                  </h2>
                  <div className="grid grid-cols-1 gap-3">
                    {!advocatePendingEffective && (focusCaseId || noCasesYet) ? (
                      ...
                      >
                        {t("applicantDashboard.applyPathConnect")}
                      </button>
                    ) : null}
                    <button
                      type="button"
                    ...
                    >
                      {t("applicantDashboard.applyPathSelf")}
                    </button>
                  </div>
                  <button
                    type="button"
                ...
                <>
                  <h3 id="apply-modal-title" className="text-base font-semibold text-[var(--color-navy)]">
                    {t("applicantDashboard.stateModalTitle")}
                  </h3>
                  <p className="text-sm leading-relaxed text-[var(--color-muted)]">{t("applicantDashboard.stateModalSubtitle")}</p>
                  ...
                  <button
                    ...
                    className="w-full text-center text-xs text-[var(--color-muted)] hover:text-[var(--color-slate)]"
                  >
                    ← {t("applicantDashboard.applyPathBack")}
                  </button>
                  <button
                    type="button"
                    disabled={creatingCase}
                ...
                <>
                  ...
                  <p className="text-sm leading-relaxed text-[var(--color-slate)]">
                    {tf("applicantDashboard.eligibleReviewIntro", {
                      state: pendingState === "IN" ? t("applicantDashboard.stateIN") : t("applicantDashboard.stateIL"),
                    })}
                  </p>
                  <button
                    type="button"
                  ...
                  >
                    {creatingCase ? t("applicantDashboard.creating") : t("applicantDashboard.continueToEligibility")}
                  </button>
                  <button
                    type="button"
                    disabled={creatingCase}
          ...
          >
            ...
            >
              <h3 id="rename-case-title" className="text-sm font-semibold text-[var(--color-navy)]">
                {t("applicantDashboard.editNameTitle")}
              </h3>
              <form
                className="space-y-3"
                onSubmit={(e) => {
              ...
              >
                <input
                  ...
                  placeholder={t("applicantDashboard.caseNamePlaceholder")}
          ...
          >
            ...
            >
              <h3 id="delete-app-title" className="text-sm font-semibold text-[var(--color-navy)]">
                {t("applicantDashboard.deleteModalTitle")}
              </h3>
              <div className="text-xs text-[var(--color-slate)] space-y-2">
                <p>{t("applicantDashboard.deleteModalBodyLine1")}</p>
                <p>{t("applicantDashboard.deleteModalBodyLine2")}</p>
              </div>
              ...
              <div className="flex flex-wrap gap-2 justify-end">
                ...
                >
                  {t("applicantDashboard.deleteModalCancel")}
                </button>
                <button
                  type="button"
                  disabled={deleteBusy}
                ...
                >
                  {t("applicantDashboard.deleteModalConfirm")}
                </button>
              </div>
            </div>
          </div>
